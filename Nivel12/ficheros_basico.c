...
int escribir_bit(unsigned int nbloque, unsigned int bit)
{
    ...
    if (bread(nbloqueabs, bufferMB) == FAILURE)
    {
        return FAILURE;
    }
    ...
    if (bwrite(nbloqueabs, bufferMB) == FAILURE)
    {
        return FAILURE;
    }
...
#if DEBUG3
    fprintf(stderr, "[leer_bit(%i) → posbyte:%i, posbit:%i, nbloqueMB:%i, nbloqueabs:%i)]\n\n", nbloque, posbyte, posbit, nbloqueMB, nbloqueabs);
#endif
...
int reservar_bloque()
{
    ...
    if (bwrite(SB.posPrimerBloqueDatos + nbloque - 1, bufferaux) == FAILURE)
    {
...
int escribir_inodo(unsigned int ninodo, struct inodo inodo)
{
    ...
    if (bread(posBloqueInodo, inodos) == FAILURE)
    {
        return FAILURE;
    }
    ...
    if (bwrite(posBloqueInodo, inodos) == FAILURE)
    {
        return FAILURE;
    }
 ...
 * 
 * In: unsigned int ninodo inodo a leer
 *     struct inodo *inodo
 * Out: EXIT_SUCCESS
 *      FAILURE
 */
...
int obtener_indice(int nblogico, int nivel_punteros)
{
    ...
    //Si nblogico > INDIRECTOS2, es un error y devolvemos FAILURE porque no podemos tratarlo.
    return FAILURE;
}
        ...
        { //no cuelgan bloques de punteros
            ...
            { //reservar bloques punteros y crear enlaces desde inodo hasta datos
                ...
                else
                {
                    buffer[indice] = ptr;
#if DEBUG4
                    fprintf(stderr, "[traducir_bloque_inodo()→ inodo.punteros_nivel%i[%i] = %i (reservado BF %i para punteros_nivel%i)]\n",
    ...
    for (nBL = primerBL; nBL <= ultimoBL; nBL++)
    {
        ...
        { //cuelgan bloques de punteros
            ...
            if ((indice == 0) || (nBL == primerBL))
            {
                ...
                if (bread(ptr, bloques_punteros[nivel_punteros - 1]) == FAILURE)
                {
            ...
            else
            {
                ...
                while (nivel_punteros <= nRangoBL)
                {
                    ...
                    { //escribimos en el dispositivo el bloque de punteros modificado
                        if (bwrite(ptr, bloques_punteros[nivel_punteros - 1]) == FAILURE)
                        {
